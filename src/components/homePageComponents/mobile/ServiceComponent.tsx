...
export function ServiceComponent(props: {service: Service, isFirst: boolean, isLast: boolean}) {
    ...
    const indexArg = featureArg.includes("Full Car Wash") ? 4 : featureArg.includes("Detailing") ? 6 : featureArg.includes("Polishing") ? 5 : featureArg.includes("Interior") ? 10 : -1

    return(
        <div key={props.service.title} className={`${props.isFirst == true ? 'ms-5' : ''} ${props.isLast == true  ? 'me-5' : ''} service bg-gradient-to-tr from-[#235999] to-[#3A94FF] text-white flex max-w-52 rounded-xl shadow-[0_1px_7px_0_rgba(0,0,0,0.7)]`}>
